 ...
 * Default locale & region: Indonesia (geo: 'ID', lang: 'id', hl: 'id-ID').
 *
...
function extractJsonFromResponse(text) {
	...
	if (!nestedJsonString)
		throw new Error('Invalid response format: missing nested JSON')
...
function extractArticles(item) {
	...
	if (!imageData || !Array.isArray(imageData) || imageData.length <= 3)
		return []
...
// --- API functions with Default Indonesia (ID) ---

async function autocomplete(keyword, hl = 'id-ID') {
	if (!keyword)
		return { data: [] }
	const options = {
...
async function dailyTrends({ geo = 'ID', lang = 'id' } = {}) {
	const options = {
...
async function realTimeTrends({ geo = 'ID', trendingHours = 4 } = {}) {
	const options = {
		...ENDPOINTS.dailyTrends,
		body: new URLSearchParams({
			'f.req': `[[["i0OFE","[null,null,\\"${geo}\\",0,\\"id\\",${trendingHours},1]",null,"generic"]]]`,
		}).toString(),
		contentType: 'form',
	}
	const response = await request(options.url, options)
...
async function explore({ keyword, geo = 'ID', time = 'now 1-d', category = 0, property = '', hl = 'id-ID', enableBackoff = false } = {}) {
	const options = {
		...ENDPOINTS.explore,
		qs: {
			hl,
...
async function interestByRegion({
	...
	if (!widget)
		throw new Error('No GEO_MAP widget found in explore response')
...
async function relatedTopics({
	...
	if (!widget)
		throw new Error('No related topics widget found')
...
async function relatedQueries({
	...
	if (!widget)
		throw new Error('No related queries widget found')
...
async function main() {
	...
	try {
		...
		switch (command) {
			case 'daily-trends':
				result = await dailyTrends({ geo: parsed.geo || 'ID', lang: parsed.lang || 'id' })
				break

			case 'realtime-trends':
				result = await realTimeTrends({ geo: parsed.geo || 'ID', trendingHours: parseInt(parsed.hours || '4', 10) })
				break
			...
			case 'autocomplete':
				...
				result = await autocomplete(keyword, parsed.hl || 'id-ID')
				break
			...
			case 'explore':
				...
				result = await explore({
					keyword,
					geo: parsed.geo || 'ID',
					time: parsed.time || 'now 1-d',
					category: parseInt(parsed.category || '0', 10),
			...
			case 'interest-by-region':
				...
				result = await interestByRegion({
					keyword,
					geo: parsed.geo || 'ID',
					resolution: parsed.resolution || 'REGION',
					hl: parsed.hl || 'id-ID',
