...
- [references/okflint.md](references/okflint.md) — Linter & Validation Guide (`okflint`)
- [examples/majalengka-design-system/](examples/majalengka-design-system/) — Real-world example bundle with `okf-base.yaml` and GitHub Actions CI workflow
...
## Manifest & Automated Lint Validation (`okflint`)

Setiap kali Anda membuat atau menstandardisasi bundle OKF, **selalu sertakan konfigurasi linter `okflint`** agar kepatuhan dan kebersihan bundle dapat diverifikasi secara otomatis:

### 1. File Manifest `okf-base.yaml` di Root Bundle
Manifest ini mendefinisikan aturan profil tipe konsep, field wajib/opsional, batasan status, dan aturan kebersihan (*hygiene*):
- Tentukan `okf_version: "0.2"`
- Konfigurasi `base.name`, `roots`, `reserved_files` (`index.md`, `log.md`), dan `link_resolution`
- Definisikan `profile.types` sesuai domain bundle (contoh: PRD, Architecture Decision, Standard, Design Token, Component Spec, Metric, Attested Computation)
- Tentukan aturan `hygiene`:
  ```yaml
  hygiene:
    broken_links: error
...
### 2. GitHub Actions CI Workflow (`.github/workflows/okflint.yml`)
Otomatisasi pengujian bundle di GitHub Actions:
```yaml
name: OKF lint
...
### 3. Perintah Eksekusi Mandiri
- Validasi Manifest: `okflint validate-manifest okf-base.yaml`
- Validasi Kepatuhan: `okflint validate --manifest okf-base.yaml`
- Audit Komprehensif: `okflint audit --manifest okf-base.yaml`

Lihat contoh bundle produksi lengkap di [`examples/majalengka-design-system/`](examples/majalengka-design-system/) dan panduan detail di [`references/okflint.md`](references/okflint.md).
...
1. **Directory tree** showing the full structure (including `okf-base.yaml` and `.github/workflows/okflint.yml`)
2. **Each file's content** in fenced code blocks
3. **Conformance check** confirming the bundle passes the 3 rules + okflint manifest
...
```
Bundle is OKF v0.2 conformant & lint-ready ✅
- 4 concept files
- 1 Attested Computation
- 3 human-verified, 1 unverified
- 0 stale concepts
- okf-base.yaml manifest & .github/workflows/okflint.yml configured
```
